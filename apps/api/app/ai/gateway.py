...
class AIGateway:
    ...
    async def chat(
        ...
        if self.provider == "ollama":
            ...
            if not self.enable_external:
                raise RuntimeError("External AI disabled. Set ENABLE_EXTERNAL_AI=true + record consent.")
            # Simplified: assume OpenAI compat or let litellm handle if installed
            # In full: from litellm import acompletion
            url = f"{self.base_url}/v1/chat/completions" if "openai" in self.provider or settings.AI_BASE_URL else "https://api.openai.com/v1/chat/completions"
            headers = {}
            if settings.OPENAI_API_KEY and "openai" in self.provider:
                headers["Authorization"] = f"Bearer {settings.OPENAI_API_KEY}"
            payload = {"model": model, "messages": redacted_messages, "temperature": temperature, "max_tokens": max_tokens, "stream": stream}
        ...
        logger.info("ai.chat", extra={
            "provider": self.provider, "model": model, "latency_ms": latency,
            "usage": usage, "user_id": user_id, "request_id": request_id, "external": self.enable_external
        })
        # TODO: persist to ai_requests table with redacted flag, consent_used, etc.

        return {"content": content, "usage": usage, "latency_ms": latency, "provider": self.provider}
    ...
    async def structured(
        ...
        result = await self.chat(messages, model=model, temperature=0.1, user_id=user_id, request_id=request_id)
        try:
            parsed = json.loads(result["content"])
            return parsed
        except Exception:
            logger.warning("structured.parse_fail", extra={"raw": result["content"][:200]})
    ...
    async def embed(self, text: str, model: Optional[str] = None) -> List[float]:
        ...
        raise NotImplementedError("Embed for external not wired in this slice; use local nomic.")
