...
Be evidence-based only from the provided text. No fabrication."""
    ...
    extracted = await gateway.structured(system, jd[:6000], schema={"type":"object"}, user_id=user_id)

    # Simple match if resume provided (in real use RAG + gateway on combined)
    resume_snippet = (req.resume_text or "User has experience in Python, LLMs, backend systems, distributed infra.")[:2000]
    ...
    result = {
        ...
        "missing_skills": match_raw.get("missing_skills", extracted.get("preferred_skills", [])[:5]),
        ...
        "rewrite_strategy": match_raw.get("rewrite_strategy", "Emphasize relevant backend + LLM experience from your actual projects. Quantify impact where you have evidence."),
        "recruiter_message_draft": match_raw.get("recruiter_message_draft", "Hi, I saw your posting for the role. My background in [your real project] aligns with the distributed systems needs. Would value a conversation."),
        "cover_letter_draft": match_raw.get("cover_letter_draft", "I am excited about the opportunity... (edit with your voice). This is a draft only."),
        "citations": match_raw.get("citations", ["user provided JD", "uploaded resume (if any)"]),
        "warnings": ["This is planning guidance only. Do not fabricate experience. Review and edit all drafts. No guarantee of interview or offer."]
    }
    return result
